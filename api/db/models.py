...
class PorterLeadModel(Base):
    __tablename__ = "porter_leads"
    ...
    funding_signal_type = Column(String, nullable=False)
    lead_source = Column(String, nullable=False)
    lead_score_before_call = Column(Float, nullable=False)
    ...
    __table_args__ = (
        Index("ix_porter_leads_company_name", "company_name"),
        Index("ix_porter_leads_lead_source", "lead_source"),
        Index("ix_porter_leads_do_not_call", "do_not_call"),
    )


class PorterPhoneSuppressionModel(Base):
    __tablename__ = "porter_phone_suppressions"

    id = Column(Integer, primary_key=True, index=True)
    phone = Column(String(16), nullable=False, unique=True)
    reason = Column(Text, nullable=False)
    source_call_session_id = Column(
        Integer,
        ForeignKey("porter_call_sessions.id", ondelete="SET NULL"),
        nullable=True,
    )
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(UTC))
    updated_at = Column(
...
class PorterPromptVersionModel(Base):
    __tablename__ = "porter_prompt_versions"
...
class PorterKnowledgeBaseChunkModel(Base):
    __tablename__ = "porter_knowledge_base_chunks"
    ...
    __table_args__ = (
        ...
        Index("ix_porter_kb_chunks_version", "knowledge_base_version"),
    )


class PorterCallSessionModel(Base):
    __tablename__ = "porter_call_sessions"

    id = Column(Integer, primary_key=True, index=True)
    lead_id = Column(
        Integer, ForeignKey("porter_leads.id", ondelete="CASCADE"), nullable=False
    )
    dograh_call_id = Column(
    ...
    destination_phone = Column(String(16), nullable=False)
    status = Column(
        String, nullable=False, default="created", server_default=sa_text("'created'")
    )
    started_at = Column(DateTime(timezone=True), nullable=True)
    ...
    crm_sync_status = Column(
        String,
        nullable=False,
        default="not_ready",
        server_default=sa_text("'not_ready'"),
    )
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(UTC))
    updated_at = Column(
        DateTime(timezone=True),
    ...
    lead = relationship("PorterLeadModel", back_populates="call_sessions")
    dograh_call = relationship("WorkflowRunModel")
    ...
    qualification_answers = relationship(
    ...
    __table_args__ = (
        ...
        Index("ix_porter_call_sessions_lead_id", "lead_id"),
        Index("ix_porter_call_sessions_dograh_call_id", "dograh_call_id"),
        Index("ix_porter_call_sessions_destination_phone", "destination_phone"),
    )


class PorterTranscriptTurnModel(Base):
    __tablename__ = "porter_transcript_turns"

    id = Column(Integer, primary_key=True, index=True)
    call_session_id = Column(
        Integer, ForeignKey("porter_call_sessions.id", ondelete="CASCADE"), nullable=False
    )
    speaker = Column(String, nullable=False)
    text = Column(Text, nullable=False)
    conversation_state = Column(String, nullable=False)
    ...
    call_session = relationship(
        "PorterCallSessionModel", back_populates="transcript_turns"
    )
...
class PorterQualificationAnswerModel(Base):
    __tablename__ = "porter_qualification_answers"

    id = Column(Integer, primary_key=True, index=True)
    call_session_id = Column(
        Integer, ForeignKey("porter_call_sessions.id", ondelete="CASCADE"), nullable=False
    )
    funding_need = Column(Text, nullable=True)
    funding_amount = Column(String, nullable=True)
    monthly_revenue = Column(String, nullable=True)
    invoices_b2b = Column(Boolean, nullable=True)
    has_outstanding_ar = Column(Boolean, nullable=True)
    has_payroll_need = Column(Boolean, nullable=True)
    has_purchase_order_need = Column(Boolean, nullable=True)
    existing_factor_or_lender = Column(String, nullable=True)
    ...
    call_session = relationship(
        "PorterCallSessionModel", back_populates="qualification_answers"
    )

    __table_args__ = (
        UniqueConstraint("call_session_id", name="uq_porter_qualification_call_session"),
    )


class PorterPolicyViolationModel(Base):
    __tablename__ = "porter_policy_violations"
    ...
    blocked_text = Column(Text, nullable=False)
    safe_replacement = Column(Text, nullable=False)
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(UTC))

    call_session = relationship(
        "PorterCallSessionModel", back_populates="policy_violations"
    )

    __table_args__ = (
        Index("ix_porter_policy_violations_session", "call_session_id"),
        Index("ix_porter_policy_violations_type", "violation_type"),
    )


class PorterAgentEventModel(Base):
    __tablename__ = "porter_agent_events"
    ...
    call_session = relationship("PorterCallSessionModel", back_populates="agent_events")

    __table_args__ = (
        Index("ix_porter_agent_events_session", "call_session_id"),
        Index("ix_porter_agent_events_type", "event_type"),
    )


class PorterCallOutcomeModel(Base):
    __tablename__ = "porter_call_outcomes"
    ...
    call_summary = Column(Text, nullable=False)
    objections = Column(
        postgresql.JSONB,
        nullable=False,
        default=list,
    ...
    human_handoff_reason = Column(Text, nullable=True)
    created_at = Column(DateTime(timezone=True), default=lambda: datetime.now(UTC))

    call_session = relationship("PorterCallSessionModel", back_populates="outcome")

    __table_args__ = (
        UniqueConstraint("call_session_id", name="uq_porter_outcome_call_session"),
        CheckConstraint(
            "disposition IN ("
        ...
        Index("ix_porter_call_outcomes_disposition", "disposition"),
    )
