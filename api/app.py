...
from api.porter.runtime import STARTUP_VOICE_SCRIPT_NAMES, prewarm_porter_voice_scripts
...
async def lifespan(app: FastAPI):
    async with mcp_app.lifespan(app):
        ...
        voice_prewarm_task = None
        if ENVIRONMENT != "test":
            voice_prewarm_task = asyncio.create_task(_prewarm_porter_voice_scripts())

        yield  # Run app

        # Shutdown sequence - this runs when FastAPI is shutting down
        logger.info("Starting graceful shutdown...")
        if voice_prewarm_task is not None and not voice_prewarm_task.done():
            voice_prewarm_task.cancel()
            try:
                await voice_prewarm_task
            except asyncio.CancelledError:
                pass
        await sync_manager.stop()


async def _prewarm_porter_voice_scripts() -> None:
    try:
        result = await prewarm_porter_voice_scripts(script_names=STARTUP_VOICE_SCRIPT_NAMES)
        logger.info(
            "Prewarmed {} Porter voice scripts in {} ms",
            result.warmed_count,
            result.total_elapsed_ms,
        )
    except Exception:
        logger.warning("Porter voice script prewarm failed; live synthesis will retry.")
