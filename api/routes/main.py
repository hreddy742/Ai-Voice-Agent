...
from api.routes.porter_local import router as porter_local_router
...
router.include_router(porter_local_router)
...
async def health() -> HealthResponse:
    ...
    from api.utils.common import get_scheme

    logger.debug("Health endpoint called")
    scheme = get_scheme(BACKEND_API_ENDPOINT)
    backend_endpoint = (
        BACKEND_API_ENDPOINT.rstrip("/")
        if scheme
        else f"http://{BACKEND_API_ENDPOINT.rstrip('/')}"
    )
