...
const isExpectedUnavailableBillingStatus = (status?: number) => (
...
export default function BillingPage() {
    ...
    const fetchCredits = useCallback(async (
        ...
        if (auth.loading || configLoading || !hasAppConfig) {
            return;
        }

        if (!auth.isAuthenticated || isOssMode) {
            setCredits(null);
            setLoading(false);
            setRefreshing(false);
            return;
        ...
        try {
            ...
            if (response.error) {
                if (isExpectedUnavailableBillingStatus(response.response?.status)) {
                    setCredits(null);
                    return;
                }
    ...
    }, [auth.isAuthenticated, auth.loading, configLoading, hasAppConfig, isOssMode]);
